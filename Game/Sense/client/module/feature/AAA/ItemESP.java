...
public class ItemESP extends Module {
    public ColorSetting itemESPColor = new ColorSetting("ItemESP Color", new Color(255, 0, 0).getRGB(), () -> true);
    private final BooleanSetting box = new BooleanSetting("Box", true, () -> true);

    public ItemESP() {
        super("ItemESP", "Отображение айтемов", ModuleCategory.RENDER);
        ...
        for (Entity entity : mc.world.loadedEntityList) {
            if (isValid(entity) && RenderUtils.isInViewFrustum(entity)) {
                ...
                if (position != null) {
                    ...
                    float textWidth = (mc.neverlose900_18.getStringWidth(entityItem.getEntityItem().getDisplayName()) * scale);
                    float tagX = (float) ((posX + diff - textWidth / 2) * scale);
                    if (box.getBoolValue()) {
                        RenderUtils.drawRect(posX, posY, endPosX, endPosY, new Color(0, 0, 0, 40).getRGB());
                    }
                    mc.neverlose900_18.drawBlurredStringWithShadow(entityItem.getEntityItem().getDisplayName(), tagX + 5, (float) posY - 10, 8, RenderUtils.injectAlpha(new Color(itemESPColor.getColorValue()), 40), itemESPColor.getColorValue());
                }
            }
        }
