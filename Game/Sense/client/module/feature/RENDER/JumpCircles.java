...
public class JumpCircles
    ...
    public JumpCircles() {
        super("JumpCircles", "\u0414\u043e\u0431\u0430\u0432\u043b\u044f\u0435\u0442 \u0430\u043d\u0438\u043c\u0430\u0446\u0438\u044e \u043f\u0440\u044b\u0436\u043a\u0430 \u043f\u043e\u0434 \u0432\u0430\u043c\u0438", ModuleCategory.RENDER);
        this.circleType = new ListSetting("Circles Type", "Default",() -> true, "Default", "Disk");
    ...
    public void onRender3D(EventRender3D event) {
        ...
        if (this.circleType.currentMode.equals("Default")) {
            for (CircleData circle : this.circles) {
                ...
                int customColor = this.circleCustomColor.getColorValue();
                int color = 0;
                ...
                float smoothAlpha = Math.max(0.0f, 1.0f - circle.getAliveTicks() / 35.0f / this.ticksToRemove.getNumberValue() * 50.0f);
                float widthValue = 10.0f - circle.getAliveTicks() / 4.0f;
                widthValue = MathHelper.clamp(widthValue, 0.1f, widthValue);
                circle.setAnimationTicks(circle.getAnimationTicks() + this.animationSpeed.getNumberValue() / 10.0f * (float)Minecraft.frameTime * 0.1f);
                this.renderCircle(circle.getVec(), (double)circle.getAnimationTicks(), widthValue, (int)this.circleSegments.getNumberValue(), new Color((float)new Color(color).getRed() / 255.0f, (float)new Color(color).getGreen() / 255.0f, (float)new Color(color).getBlue() / 255.0f, smoothAlpha).getRGB(), smoothAlpha);
            }
            ...
            float time = this.ticksToRemove.getNumberValue();
            for (CircleDisk c : this.circleDisk) {
                ...
                for (int i = 0; i < 361; i += 5) {
                    GlStateManager.color((float)c.color().xCoord, (float)c.color().yCoord, (float)c.color().zCoord, 0.2f * (1.0f - (float)c.existed / time));
                    GL11.glVertex3d(x + Math.cos(Math.toRadians(i * 4)) * (double)start, y, z + Math.sin(Math.toRadians(i * 4)) * (double)start);
                    GlStateManager.color(1.0f, 1.0f, 1.0f, this.diskAlpha.getNumberValue() * (1.0f - (float)c.existed / time));
                    GL11.glVertex3d(x + Math.cos(Math.toRadians(i)) * (double)end, this.diskAnimation.getBoolValue() ? y + Math.sin(k * 8.0f) * 0.5 : y, z + Math.sin(Math.toRadians(i) * (double)end));
    ...
    public void onUpdate(EventUpdate event) {
        ...
        this.circles.removeIf(circleData -> ((CircleData)circleData).canRemove((int)this.ticksToRemove.getNumberValue()));
    ...
    private void renderCircle(Vec3d vec, double radius, double width, int points, int color, float alpha) {
        int i = 0;
        while ((float)i < this.loopSize.getNumberValue()) {
            this.renderCircle(vec, radius, points, (float)width, color, alpha);
            radius += (double)this.radiusAdd.getNumberValue();
            ++i;
        }
    }
    ...
    private void renderCircle(Vec3d vec, double radius, int points, float width, int color, float alpha) {
        ...
        GL11.glDisable(2929);
        GL11.glBegin(3);
        double x = vec.xCoord - JumpCircles.mc.getRenderManager().renderPosX;
        double y = vec.yCoord - 0.4 - JumpCircles.mc.getRenderManager().renderPosY;
        double z = vec.zCoord - JumpCircles.mc.getRenderManager().renderPosZ;
    ...
    class CircleDisk {
        ...
        boolean update() {
            ...
            return (float)by > JumpCircles.this.ticksToRemove.getNumberValue();
        }
    }
}
