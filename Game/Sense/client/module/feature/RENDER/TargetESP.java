...
public class TargetESP extends Module {
    ...
    public TargetESP() {
        super("TargetESP", "Рисует красивый круг на энтити", ModuleCategory.RENDER);
    ...
    public void onRender(EventRender3D event3D) {
        ...
        if (bebraPonyxana.currentMode.equals("Jello") && !KillAura.target.isDead) {
            if (KillAura.mc.player.getDistanceToEntity(KillAura.target) <= KillAura.range.getNumberValue()) {
                if (KillAura.target != null && GameSense.instance.featureManager.getFeature(KillAura.class).isEnabled() && KillAura.mc.player.getDistanceToEntity(KillAura.target) <= KillAura.range.getNumberValue()) {
                    double iCos;
                    double iSin;
                    int i;
                    ...
                    float endYValue = (float) ((float) (((GameSense.instance.featureManager.getFeature(CustomModel.class).isEnabled() && CustomModel.modelMode.currentMode.equals("Amogus") && KillAura.target instanceof EntityPlayer && !CustomModel.onlyMe.getBoolValue() ? 1.3 : (double) KillAura.target.height)) * 1.0 + 0.2) * (double) targetHeight);
