    ...
    public void drawComponent(ScaledResolution scaledResolution, int mouseX, int mouseY) {
        if (this.booleanSetting.isVisible()) {
            ...
            if (ClickGUI.theme.currentMode.equals("Light")) {
                this.mc.neverlose900_14.drawString(this.getName(), (float)(x + 3), (float)(y +  middleHeight - 2), Color.BLACK.getRGB());
            }

            if (ClickGUI.theme.currentMode.equals("Dark")) {
                if (!hovered) {
                    this.mc.neverlose900_14.drawString(this.getName(), (float) (x + 3), (float) (y + middleHeight - 2), Color.LIGHT_GRAY.getRGB());
                }
                if (hovered) {
                    this.mc.neverlose900_14.drawString(this.getName(), (float) (x + 3), (float) (y + middleHeight - 2), Color.WHITE.getRGB());
                }
            }
            if (ClickGUI.theme.currentMode.equals("Weird")) {
                this.mc.neverlose900_14.drawString(this.getName(), (float)(x + 3), (float)(y + middleHeight - 3), Color.WHITE.getRGB());
            }
