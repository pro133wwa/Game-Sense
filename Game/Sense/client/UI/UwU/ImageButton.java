...
import Game.Sense.client.UI.NursultanGui.GuiCapeSelector;
import Game.Sense.client.UI.NursultanGui.GuiConfig;
...
//            }
            if (this.target == 30 && (chest = (GuiChest)this.mc.currentScreen) != null) {
                GuiChest finalChest1 = chest;
                new Thread(() -> {
                    try {
                        for (int i = 0; i < finalChest1.getInventoryRows() * 9; ++i) {
            ...
            if (this.target == 31 && (chest = (GuiChest)this.mc.currentScreen) != null) {
                GuiChest finalChest = chest;
                new Thread(() -> {
                    try {
                        for (int i = finalChest.getInventoryRows() * 9; i < finalChest.getInventoryRows() * 9 + 44; ++i) {
                            Slot slot = finalChest.inventorySlots.inventorySlots.get(i);
                            if (slot.getStack() == null) continue;
                            Thread.sleep(50L);
                            finalChest.handleMouseClick(slot, slot.slotNumber, 0, ClickType.QUICK_MOVE);
                        }
                    }
                    catch (Exception exception) {
                        // empty catch block
    ...
    public void draw(int mouseX, int mouseY, Color color) {
        ...
        if (this.ani > 0.0f) {
            RenderUtils.drawImage(this.image, (float)this.x - this.ani, (float)this.y - this.ani, (float)(this.width + 4) + this.ani * 2.0f, (float)(this.height + 4) + this.ani * 2.0f, Color.BLACK);
            RenderUtils.drawImage(this.image, (float)this.x - this.ani, (float)this.y - this.ani, (float)this.width + this.ani * 2.0f, (float)this.height + this.ani * 2.0f, this.isHovered(mouseX, mouseY) ? new Color(156, 156, 156, 255) : Color.WHITE);
            if (this.isHovered(mouseX, mouseY)) {
            ...
            RenderUtils.drawImage(this.image, this.x, this.y, this.width + 3, this.height + 3, Color.BLACK);
