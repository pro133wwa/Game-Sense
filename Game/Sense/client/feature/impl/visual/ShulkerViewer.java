...
import Game.Sense.client.ui.newalt.EventRenderToolTip;
...
public class ShulkerViewer extends Feature {
    public ShulkerViewer() {
        super("ShulkerViewer", "ѕозвол€ет смотреть содержимое шалкера", FeatureCategory.Visuals);
    }
    ...
    public void onRenderToolTip(EventRenderToolTip event) {
        ...
        if (event.getStack().getItem() instanceof ItemShulkerBox && (tagCompound = event.getStack().getTagCompound()) != null && tagCompound.hasKey("BlockEntityTag", 10) && (blockEntityTag = tagCompound.getCompoundTag("BlockEntityTag")).hasKey("Items", 9)) {
            ...
            int width = Math.max(144, Helper.mc.neverlose900_20.getStringWidth(event.getStack().getDisplayName()) + 3);
            ...
            Helper.mc.neverlose900_20.drawStringWithShadow(event.getStack().getDisplayName(), event.getX() + 12, event.getY() - 12, 0xFFFFFF);
            GlStateManager.enableBlend();
