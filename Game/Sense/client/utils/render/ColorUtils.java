...
public class ColorUtils implements Helper {
    ...
    public static Color fade3(int speed, int index, Color color, float alpha) {
        /* 234 */     float[] hsb = Color.RGBtoHSB(color.getRed(), color.getGreen(), color.getBlue(), null);
        /* 235 */     int angle = (int)((System.currentTimeMillis() / speed + index) % 360L);
        /* 236 */     angle = ((angle > 180) ? (360 - angle) : angle) + 180;
        /*     */
        /* 238 */     Color colorHSB = new Color(Color.HSBtoRGB(hsb[0], hsb[1], angle / 360.0F));
        /*     */
        /* 240 */     return new Color(colorHSB.getRed(), colorHSB.getGreen(), colorHSB.getBlue(), Math.max(0, Math.min(255, (int)(alpha * 255.0F))));
        /*     */   }
    ...
    public static Color rainbow(int speed, int index, float saturation, float brightness, float opacity) {
        /* 185 */     int angle = (int)((System.currentTimeMillis() / speed + index) % 360L);
        /* 186 */     float hue = angle / 360.0F;
        /* 187 */     Color color = new Color(Color.HSBtoRGB(hue, saturation, brightness));
        /* 188 */     return new Color(color.getRed(), color.getGreen(), color.getBlue(), Math.max(0, Math.min(255, (int)(opacity * 255.0F))));
        /*     */   }
