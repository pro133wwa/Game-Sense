...
vec3 render(vec2 p)
{
	...
	if (t < tmax)
	{
		...
		float difference = 0.2;//clamp(dot(normal, vec3(0.57703)), 0.0, 1.0);
		float amb = 0.5 + 0.5 * dot(normal, vec3(0.0, 0.0, 1.0));
		color = hsv2rgb_smooth(vec3(abs(4.-rd.y*0.),0.6,.7)) * amb + vec3(0.8, 0.7, 0.5) * difference;
	}
